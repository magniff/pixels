...
#[test]
fn a_patch_is_applied_by_the_lines_it_quotes_not_the_numbers_it_gives() {
    use notes::patch::apply;
    let note = "# Shop\n\n- milk 2.50\n- bread 1.80\n- tofu 3.20\n- cheese 4.00\n";
    // Two hunks, far apart, the way the model is shown changes.
    let patch = "@@ -3,2 +3,2 @@\n-- milk 2.50\n+- milk 2.20\n - bread 1.80\n@@ -6,1 +6,2 @@\n - cheese 4.00\n+- eggs 3.00";
    let after = apply(note, patch).unwrap();
    assert_eq!(
        after,
        "# Shop\n\n- milk 2.20\n- bread 1.80\n- tofu 3.20\n- cheese 4.00\n- eggs 3.00\n"
    );

    // The numbers are wrong and the lines are right: the lines win.
    let off = "@@ -20,2 +20,2 @@\n-- milk 2.50\n+- milk 2.20\n - bread 1.80";
    assert!(apply(note, off).unwrap().contains("milk 2.20"));
    // No numbers at all, and a context line without its leading space,
    // which is what a model writes half the time. (A list item, which
    ...
    let twice = "# T\n\n- x\n- y\n- x\n- y\n";
    let second = "@@ -5,1 +5,1 @@\n-- x\n+- z";
    assert_eq!(apply(twice, second).unwrap(), "# T\n\n- x\n- y\n- z\n- y\n");
}

#[test]
fn a_patch_block_is_offered_and_applied_as_the_note_patched() {
    use notes::chat::{proposals, Change, Folder, What};
    let said = "Both at once.\n<patch file=\"shop.md\">@@ -3,1 +3,1 @@\n-- milk 2.50\n+- milk 2.20\n@@ -5,1 +5,1 @@\n-- tofu 3.20\n+- tofu 3.00</patch>";
    let (prose, changes) = proposals(said);
    assert_eq!(prose, "Both at once.");
    assert_eq!(changes.len(), 1);
    assert!(matches!(changes[0].what, What::Patch { .. }));
    ...
    // Offered with what it replaces and what it leaves, so the panel draws
    // the difference like any other change.
    assert!(changes[0].replacing(&folder).is_some());
    let becoming = changes[0].becoming(&folder);
    assert!(
        becoming.contains("milk 2.20") && becoming.contains("tofu 3.00"),
        "{becoming}"
    );
    assert_eq!(changes[0].tally(&folder), (2, 2));
    assert_eq!(changes[0].headline("x"), "PATCH  shop.md");
    // One that does not fit is not offered.
    let bad = Change {
        what: What::Patch {
            text: "@@ -1,1 +1,1 @@\n-# Menu\n+# Shop".into(),
        },
    ...
    assert!(bad.replacing(&folder).is_none());

    // Applied, the note is the note patched.
    let dir = std::env::temp_dir().join(format!("notes-patch-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).expect("a vault");
    std::fs::write(dir.join("shop.md"), lines.join("\n")).expect("a note");
    let mut app = notes::Notes::open(dir.clone());
    app.current = app
        .notes
        .iter()
    ...
    let text = app.notes[app.current].buffer.to_text();
    assert!(
        text.contains("milk 2.20") && text.contains("tofu 3.00") && text.contains("bread 1.80"),
        "{text}"
    );
    app.apply_change(&bad);
    assert!(
        app.status.starts_with("THE PATCH DOES NOT FIT"),
        "{}",
        app.status
    );
    let _ = std::fs::remove_dir_all(&dir);
